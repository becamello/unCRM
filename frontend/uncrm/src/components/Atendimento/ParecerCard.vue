<template>
  <v-card class="mb-3 parecer" elevation="0">
    <v-card-text>
      <v-card-actions class="d-flex align-center justify-space-between pa-0 pb-4">
        <p class="ma-0">{{ parecer.usuarioCriadorLogin }}</p>
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            <span v-bind="attrs" v-on="on">
              <v-btn
                icon
                v-if="Number(parecer.usuarioCriadorId) === Number(usuarioLogadoId)"
